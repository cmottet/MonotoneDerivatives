...
load("data/MaxConvexTransformedHorrorDist_TailProb.Rdata")

optim = subset(optim, ParamCover) #&ParamCover
...
convex_Data = transform(optim_convex,
          nparam = as.factor(rowSums(optim_convex[,1:N])),
          J1empty = rowSums(optim_convex[,(NJ2+1):N])==0,
          J2empty = rowSums(optim_convex[,1:NJ1])==0,
          RelErr = with(optim_convex,apply(direction*(cbind(lB,uB) - (1-P))/(1-P),1 ,min)),
          D  = as.factor(D),
          Est = rowSums(optim_convex[,1:NJ2]) >= 1,
          param.inc = apply(optim_convex[,1:N], 1,function(mat){paste(subset(ParamNames,unlist(mat)),collapse = ",")})
)


maxErrRel <- expand.grid(D = 0:5, P = P) %>% transform(maxErrRel = P/(1-P))

plot <- ggplot(data = subset(Data,direction ==1 & eps < 1e-4), aes(x = D, y = RelErr,text = param.inc,size = J1empty)) +
...
plot +   geom_point(data = convex_Data, aes(x = D, y = RelErr), col = "red") 

ggplot(data = convex_Data, aes(x = D, y = RelErr)) +
  geom_point(col="red") +
  geom_hline(data = maxErrRel, aes(yintercept = maxErrRel))+
  geom_point(data = convex_Data, aes(x = D, y = RelErr), col = "red") + 
  facet_wrap(~P,ncol = 4,nrow = 4,
             labeller = as_labeller(c("0.9" = "p = 90",
                                      "0.99" = " p = 99",
                                      "0.999" = " p = 99.9", 
