...
q <-  log(qhorror(p))
x <- log(seq(qhorror(0),1400,by = 0.1))

Data <- data.frame(X = log(sample))
Data2 <- data.frame(X = x, Y = dhorror(exp(x))*exp(x))
...
ggsave("pics/Histogram_Log_Horror_Plot.png",width =5,height = 5) # Save plot in png
